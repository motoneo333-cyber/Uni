...
export default function OcclusionEditor({
  ...
  const [question, setQuestion] = useState("¿Qué estructura es?");
  ...
  async function saveRegion(e: React.FormEvent) {
    e.preventDefault();
    if (!pending || !imageUrl || !label.trim()) return;
    setSaving(true);
    try {
      const res = await fetch(`/api/subjects/${subjectId}/cards`, {
        ...
        body: JSON.stringify({
          front: question.trim() || "¿Qué estructura es?",
          back: label.trim(),
    ...
    return (
      <div>
        <p className="text-xs text-zinc-500 mb-2">
          Subí una imagen (ej. un diagrama anatómico) para empezar a marcar zonas.
        </p>
        <input
          type="file"
  ...
  return (
    <div className="space-y-3">
      <p className="text-xs text-zinc-500">
        Marcá con el mouse (click y arrastrá) la zona que querés tapar, escribí qué es y
        agregala. Podés repetir sobre la misma imagen para crear varias tarjetas.
      </p>
      ...
      {pending && (
        ...
        >
          <input
            ...
            placeholder="Pregunta (se repite para todas las zonas de esta imagen)"
          ...
          <input
            ...
            placeholder="¿Qué es esta zona? (va a ser la respuesta)"
            className="w-full rounded-md border border-zinc-300 px-2 py-1 text-sm"
          />
      ...
      <div className="flex items-center justify-between text-xs">
        <span className="text-zinc-500">
          {regions.length} zona{regions.length === 1 ? "" : "s"} agregada
          {regions.length === 1 ? "" : "s"} en esta imagen
        </span>
        <button
          type="button"
          onClick={() => {
